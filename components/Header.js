...
class Header extends React.Component {
    ...
    return (
      <Block row style={styles.options}>
        <Button  style={[styles.tab]} onPress={() => navigation.navigate('Pro')}>
          <Block row middle>
            <Icon name="message" family="other" style={{ paddingRight: 8 }} color={argonTheme.COLORS.ICON} />
            <Text size={16} style={styles.tabTitle}>{optionLeft || 'Noticias'}</Text>
          </Block>
        </Button>
      </Block>
    );
  ...
  renderHeader = () => {
    var { search, options, tabs } = this.props;
    search=false;
...
const styles = StyleSheet.create({
  ...
  navbar: {
    ...
    paddingTop: iPhoneX ? theme.SIZES.BASE * 2.8 : theme.SIZES.BASE,
    zIndex: 5,
  },
  navbarSecondary: {
    paddingVertical: 0,
